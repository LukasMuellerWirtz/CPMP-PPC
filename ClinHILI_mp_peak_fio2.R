...
# Assess distribution of CP with a histogram
ggplot(data, aes(x = twa_cp)) +
  geom_histogram(binwidth = (max(data$twa_cp) - min(data$twa_cp)) / 20, # Approximate 50 bins
                 fill = "skyblue",        # Bar color
                 color = "white",         # Border color of bars
                 boundary = 0) +          # Align bins to integer values
  labs(title = "Chemical Power",     # Title of the plot
       x = "Time-weighted average chemical power [J/min]",                      # X-axis label
       y = "Frequency") +                 # Y-axis label
  scale_x_continuous(breaks = seq(6, 18, by = 1)) +  # More x-axis ticks
  theme_minimal()                         # Use minimal theme


# Assess distribution of FiO2 with a histogram
ggplot(data, aes(x = twa_fio2)) +
  geom_histogram(binwidth = (max(data$twa_fio2) - min(data$twa_fio2)) / 50, # Approximate 50 bins
                 fill = "skyblue",        # Bar color
                 color = "white",         # Border color of bars
                 boundary = 0) +          # Align bins to integer values
  labs(title = "FiO2",     # Title of the plot
       x = "Time-weighted average FiO2",                      # X-axis label
       y = "Frequency") +                 # Y-axis label
  scale_x_continuous(breaks = seq(30, 100, by = 2)) +  # More x-axis ticks
  theme_minimal()                         # Use minimal theme

# Count cases with non-titrated FiO2 
(count_fio2_40 <- sum(data$twa_fio2 == 40, na.rm = TRUE))
(count_fio2_50 <- sum(data$twa_fio2 == 50, na.rm = TRUE))
(count_fio2_80 <- sum(data$twa_fio2 == 80, na.rm = TRUE))
(count_fio2_or <- sum(data$twa_fio2 == 40 | data$twa_fio2 == 50, na.rm = TRUE | data$twa_fio2 == 80, na.rm = TRUE))

# Exclude cases with titrated FiO2
data <- data %>% 
  filter(twa_fio2 == 40 | twa_fio2 == 50 | twa_fio2 == 80)
...
# save table 0
t1flex(tbl0) %>% 
  save_as_docx(path="mp_peak_fio2_revised/tables/table0.docx")
...
ggsave("mp_peak_fio2_revised/figures/figure_power_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
# save the combined plot
ggsave("mp_peak_fio2_revised/figures/figure_power.jpg", units="cm", width=15, height=10, dpi=300)
...
# save table 1
t1flex(tbl1) %>% 
  save_as_docx(path="mp_peak_fio2_revised/tables/table1.docx")
...
# save table 2
t1flex(tbl2) %>% 
  save_as_docx(path="mp_peak_fio2_revised/tables/table2.docx")
...
jpeg(filename = "mp_peak_fio2_revised/figures/correlation_plot.jpg",  width = 1000, height = 800, quality = 95)
...
save_as_docx(regression_flextable, path = "mp_peak_fio2_revised/tables/regression_table_linear_model_interaction.docx")
...
# save regression table
regression_flextable <- as_flex_table(regression_table)
save_as_docx(regression_flextable, path = "mp_peak_fio2_revised/tables/regression_table_linear_model.docx")
...
ggsave("mp_peak_fio2_revised/figures/figure_mp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_peak_fio2_revised/figures/figure_cp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_peak_fio2_revised/figures/figure_cp_mp_ppc.jpg", units="cm", width=30, height=10, dpi=300)


# FIXED INTERVAL BINNING and HEATMAP

# Define bin edges
mp_bin <- c(min(data$twa_mp_peak), 10, 18, max(data$twa_mp_peak))
cp_bin <- c(min(data$twa_cp), 8, 12, max(data$twa_cp))
...
# define bin labels
mp_bin_labels <- c("<10", "10-18", ">18")
cp_bin_labels <- c("7", "9", "16")
...
ggsave("mp_peak_fio2_revised/figures/figure_interaction_fixed_heatmap.jpg", units="cm", width=15, height=10, dpi=300)
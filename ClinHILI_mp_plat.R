...
data <- data[!is.na(data$twa_mp_plat) & !is.na(data$twa_cp), ]

# exclude cases with mp > 30 J/min
data <- data[data$twa_mp_plat < 30 & data$twa_mp_plat > 0, ]
...
data$quartile_cp[data$twa_cp >  0              & data$twa_cp <= cp_quartile_25  & !is.na(data$twa_cp)] <- 1
data$quartile_cp[data$twa_cp >  cp_quartile_25 & data$twa_cp <= cp_quartile_50  & !is.na(data$twa_cp)] <- 2
data$quartile_cp[data$twa_cp >  cp_quartile_50 & data$twa_cp <= cp_quartile_75  & !is.na(data$twa_cp)] <- 3
data$quartile_cp[data$twa_cp >  cp_quartile_75 & data$twa_cp <= cp_quartile_100 & !is.na(data$twa_cp)] <- 4
...
var.list <- c("twa_mp_plat", "twa_cp", "age", "gender", "bmi", "asa_score", 
...
tbl0 <- table1( ~ twa_cp + twa_mp_plat, 
                data = data,
                caption = "Intraoperative chemical and mechanical power")

# save table 0
t1flex(tbl0) %>% 
  save_as_docx(path="mp_plat_revised/tables/table0.docx")
...
ggsave("mp_plat_revised/figures/figure_power_ppc.jpg", units="cm", width=15, height=10, dpi=300)


# Reshape the data to long format
data_long <- data %>%
  pivot_longer(cols = c(twa_cp, twa_mp_plat), 
               names_to = "variable", 
               values_to = "power")
...
combined_plot <- ggplot(data_long, aes(x = variable, y = power)) + 
  geom_boxplot() +
  scale_x_discrete(labels = c("twa_cp" = "Chemical Power", "twa_mp_plat" = "Mechanical Power")) +
  labs(x = "", y = expression("Power [J." * min^{-1} * "]"), title = "") + 
  theme_minimal()

# save the combined plot
ggsave("mp_plat_revised/figures/figure_power.jpg", units="cm", width=15, height=10, dpi=300)
...
# save table 1
t1flex(tbl1) %>% 
  save_as_docx(path="mp_plat_revised/tables/table1.docx")
...
# save table 2
t1flex(tbl2) %>% 
  save_as_docx(path="mp_plat_revised/tables/table2.docx")
...
jpeg(filename = "mp_plat_revised/figures/correlation_plot.jpg",  width = 1000, height = 800, quality = 95)
corrplot(cor_matrix, method = "circle", type = "upper", tl.col = "black", tl.cex = 0.8)
dev.off()

# adjusted model
prim.model.adj.int <- glm(ppc ~ twa_mp_plat * twa_cp + 
                        age + 
                        gender + 
                        bmi +
                        asa_score +
...
save_as_docx(regression_flextable, path = "mp_plat_revised/tables/regression_table_linear_model_interaction.docx")

# adjusted model
prim.model.adj <- glm(ppc ~ twa_mp_plat + twa_cp + 
                        age + 
                        gender + 
                        bmi +
                        asa_score +
...
# save regression table
regression_flextable <- as_flex_table(regression_table)
save_as_docx(regression_flextable, path = "mp_plat_revised/tables/regression_table_linear_model.docx")

# predict using the model
data$predictions <- predict(prim.model.adj, newdata = data, type = "response")

# plot predictions over mp
(mp_ppc <- ggplot(data, aes(twa_mp_plat, predictions)) +
...
ggsave("mp_plat_revised/figures/figure_mp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_plat_revised/figures/figure_cp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_plat_revised/figures/figure_cp_mp_ppc.jpg", units="cm", width=30, height=10, dpi=300)

# FIXED INTERVAL BINNING and HEATMAP

# Define bin edges
mp_bin <- c(min(data$twa_mp_plat), 12, 21, max(data$twa_mp_plat))
cp_bin <- c(min(data$twa_cp), 10, 15, max(data$twa_cp))

# Define bin edges
#mp_bin <- c(min(data$twa_mp_plat), 12, 20, max(data$twa_mp_plat))
#cp_bin <- c(min(data$twa_cp), 10, 15, max(data$twa_cp))

# Bin the data
data.binned <- data %>%
  mutate(mp_bin = cut(twa_mp_plat, breaks = mp_bin, include.lowest = TRUE),
...
# Plot heatmap with fixed intervals
#mp_bin_labels <- levels(bin_summary$mp_bin)  # or calculate midpoints based on bin edges
#cp_bin_labels <- levels(bin_summary$cp_bin)

mp_bin_labels <- c("<12", "12-21", ">21")
cp_bin_labels <- c("<10", "10-15", ">15")

ggplot(bin_summary, aes(x = mp_bin, y = cp_bin, fill = probabilities)) +
  geom_tile() +
...
ggsave("mp_plat_revised/figures/figure_interaction_fixed_heatmap.jpg", units="cm", width=15, height=10, dpi=300)
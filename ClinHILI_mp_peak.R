...
# save table 0
t1flex(tbl0) %>% 
  save_as_docx(path="mp_peak_revised/tables/table0.docx")

# cumulative frequency plots for mechanical 
# Define linetypes for PPC groups
lt_values <- c("0" = "solid", "1" = "dotted")

plot1 <- ggplot(data, aes(x = twa_mp_peak, linetype = factor(ppc))) + 
  stat_ecdf(geom = "step", size = 1, color = "black") +
  ...
  labs(
    x = expression("Mechanical Power [J." * min^{-1} * "]"),
    y = "Cumulative Frequency"
  ) +
  theme_minimal()

plot2 <- ggplot(data, aes(x = twa_cp, linetype = factor(ppc))) + 
  stat_ecdf(geom = "step", size = 1, color = "black") +
  scale_linetype_manual(
    values = lt_values,
    labels = c("0" = "No", "1" = "Yes"),
  ...
  labs(
    x = expression("Chemical Power [J." * min^{-1} * "]"),
    y = "Cumulative Frequency"
  ) +
  theme_minimal()
...
ggsave("mp_peak_revised/figures/figure_power_ppc_ecdf_bw_dotted.jpg", plot = combined_plot, units = "cm", width = 15, height = 10, dpi = 300)
...
ggsave("mp_peak_revised/figures/figure_power_ppc.jpg", units="cm", width=15, height=10, dpi=300)

# cp plot with corresponding fio2
plot1 <- ggplot(data, aes(x = ppc, y = twa_cp)) + 
  geom_boxplot(aes(group = ppc)) + 
  scale_x_discrete(labels = c("0" = "no", "1" = "yes")) +
  labs(x = "PPC", y = expression("Chemical Power [J." * min^{-1} * "]"), title = "") + 
  theme_minimal()

plot2 <- ggplot(data, aes(x = ppc, y = twa_fio2)) + 
  geom_boxplot(aes(group = ppc)) + 
  scale_x_discrete(labels = c("0" = "no", "1" = "yes")) +
  labs(x = "PPC", y = expression(Fi * O[2]), title = "") + 
  theme_minimal()
...
# save the combined plot
ggsave("mp_peak_revised/figures/figure_cp_fio2_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
# save the combined plot
ggsave("mp_peak_revised/figures/figure_power.jpg", units="cm", width=15, height=10, dpi=300)
...
# save table 1
t1flex(tbl1) %>% 
  save_as_docx(path="mp_peak_revised/tables/table1.docx")
...
# save table 2
t1flex(tbl2) %>% 
  save_as_docx(path="mp_peak_revised/tables/table2.docx")
...
jpeg(filename = "mp_peak_revised/figures/correlation_plot.jpg",  width = 1000, height = 800, quality = 95)
...
save_as_docx(regression_flextable, path = "mp_peak_revised/tables/regression_table_linear_model_interaction.docx")
...
# save regression table
regression_flextable <- as_flex_table(regression_table)
save_as_docx(regression_flextable, path = "mp_peak_revised/tables/regression_table_linear_model.docx")
...
ggsave("mp_peak_revised/figures/figure_mp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_peak_revised/figures/figure_cp_ppc.jpg", units="cm", width=15, height=10, dpi=300)
...
ggsave("mp_peak_revised/figures/figure_cp_mp_ppc.jpg", units="cm", width=30, height=10, dpi=300)
...
ggsave("mp_peak_revised/figures/figure_interaction_fixed_heatmap.jpg", units="cm", width=15, height=10, dpi=300)
ggsave("mp_peak_revised/figures/figure_interaction_quantile_heatmap_4x4.jpg", units="cm", width=15, height=10, dpi=300)